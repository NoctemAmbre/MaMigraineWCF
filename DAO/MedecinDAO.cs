...
namespace MigraineCSMiddleware.DAO
{
    public class MedecinDAO
    {
        ...
        public List<Medecin> GetListMedecin()
        {
            using (DataClasses1DataContext entity = new DataClasses1DataContext())
            {
                var ret = entity.T_MEDECIN.Join(entity.T_COMPTE,
                    M => M.IdCompte,
                    C => C.ID,
                    (P, C) => new { IDMedecin = P.ID, Identifiant = C.Identifiant, Nom = C.Nom, Prenom = C.Prenom });
        ...
        public bool IsMedecin(int idMedecin)
        {
            using (DataClasses1DataContext entity = new DataClasses1DataContext())
            {
                var retour = entity.T_MEDECIN.FirstOrDefault(elt => elt.ID == idMedecin);
        ...
        public bool IsMedecin(string Identifiant)
        {
            using (DataClasses1DataContext entity = new DataClasses1DataContext())
            {
                var retour = entity.T_MEDECIN.Join(entity.T_COMPTE,
                    M => M.IdCompte,
                    C => C.ID,
                    (M, C) => new {Identifiant = C.Identifiant, IdCompte = C.ID, IdMedecin = M.ID}).FirstOrDefault(elt => elt.Identifiant == Identifiant);
                if (retour == null) return false;
                else return true;
            }
        }
