...
namespace MigraineCSMiddleware.DAO
{
    public class PatientDAO
    {
        ...
        public Patient Login(string Identifiant, string Pass)
        {
            ...
            Rafraichir();
            Patient retourPatient = _ListPatient.SingleOrDefault(patient => patient.ID == IdCompte); //on récupère le patient à partir de l'id du compte
        ...
        public Patient Modification(Patient patient)
        {
            using (DataClasses1DataContext entity = new DataClasses1DataContext())
            {
                ...
                int retour = entity.ModifPatient(patienbdd.ID, patient.Nom, patient.Prenom, ConvertionDate.ConvertionDateTimeVersString(DateTime.UtcNow), patient.AdresseMail, ServiceSecurite.GenererToken(patient.Identifiant, new CompteDAO().GetMotDePass(patient.Identifiant), DateTime.UtcNow.Ticks), patient.IDPatient, ConvertionDate.ConvertionDateTimeVersString(patient.DateNaissance), patient.TelephonePortable, patient.Telephone, patient.Sexe);
                if (retour == -1) throw new CompteModificationException(patient, "La modification du patient n'a pus avoir lieu");
          
                Rafraichir();
                Patient RetourPatient = _ListPatient.Where(elt => elt.IDPatient == patient.IDPatient).SingleOrDefault();
        ...
        public Patient NouveauMotDePass(Patient patient)
        {
            ...
            int retour = new CompteDAO().ChangementInformation(patienbdd.ID, patient.Identifiant, MotDePasse, null, null, null, null, 0, null, ServiceSecurite.GenererToken(patient.Identifiant, MotDePasse, DateTime.UtcNow.Ticks)); //on transmet l'information de d'Id du compte et on transmet a CompteDAO l'ordre de changer le mot de passe. puis on récupère l'information que l'opération c'est bien passé
        ...
        /// <param name="idMedecin"></param>
        /// <returns></returns>
        public List<Patient> SupprMedecinDuPatient(int IdPatient, int idMedecin)
        {
            using (DataClasses1DataContext entity = new DataClasses1DataContext())
            {
                Patient patient = VoirPatient(IdPatient);
                Medecin medecin = patient.MesMedecin.SingleOrDefault(elt => elt.IDMedecin == idMedecin);
                if (patient == null) throw new PatientIncorectException("L'Id " + IdPatient.ToString() + " ne correspond pas à un patient");
                if (medecin == null) throw new MedecinNonAttribueAuPatientException("Aucun médecin n'est pas rattaché à ce patient");
                ...
                {
                    //Rafraichir();
                    return ListPatientDuMedecin(idMedecin);
                }
                return null;
            }
        }
